...
@section('content')
            ...
            <div class="d-flex justify-content-between flex-wrap">
                <div class="d-flex align-items-end flex-wrap">
                    <div class="me-md-3 me-xl-5">
                        <h2>LAPORAN PPID - Edit data</h2>
                        <div class="d-flex">
                            <i class="mdi mdi-file menu-icon"></i>
                            <p class="text-muted mb-0 hover-cursor">&nbsp;/&nbsp;Laporan&nbsp;/&nbsp;</p>
                            <p class="text-muted mb-0 hover-cursor">&nbsp;/&nbsp;LAPORAN PPID&nbsp;/&nbsp;</p>
    ...
    <div class="card">
        <div class="card-body">
            <form id="form" action="{{ route('laporan_ppid.update', ['laporan_ppid' => $laporanppid->id]) }}" method="POST" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                <div class="form-group">
                    <label for="judul">Judul</label>
                    <input type="text" class="form-control" id="judul" placeholder="Masukkan Judul"
                        name="judul" value="{{ old('judul', $laporanppid->judul) }}" required>
                    @error('judul')
                ...
                <div class="form-group">
                    ...
                    <input type="text" class="form-control" id="penerbit" placeholder="Masukkan Penerbit"
                        name="penerbit" value="{{ old('penerbit', $laporanppid->penerbit) }}" required>
                    @error('penerbit')
                        <div class="alert alert-danger">{{ $message }}</div>
                    @enderror
                </div>
                <div class="form-group">
                    ...
                    <input type="text" class="form-control" id="tahun" placeholder="Masukkan Tahun"
                        name="tahun" value="{{ old('tahun', $laporanppid->tahun) }}" required>
                    @error('tahun')
                        <div class="alert alert-danger">{{ $message }}</div>
                    @enderror
                </div>
                <div class="form-group">
                    ...
                    <input type="file" class="form-control form-control-file" id="imageInput" name="sampul" accept="image/png, image/jpeg">
                    <small class="form-text text-muted">Kosongkan jika tidak ingin mengganti sampul. Hanya file gambar (JPG/JPEG/PNG) yang diperbolehkan, ukuran maksimal 2 MB.</small>
                    @error('sampul')
                        <div class="alert alert-danger">{{ $message }}</div>
                    @enderror
                </div>
