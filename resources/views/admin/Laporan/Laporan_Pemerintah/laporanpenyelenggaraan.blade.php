    ...
    <div class="row">
            ...
            <div class="d-flex justify-content-between flex-wrap">
                <div class="d-flex align-items-end flex-wrap">
                    <div class="me-md-3 me-xl-5">
                        <h2>Laporan Penyelenggaraan Pemerintah</h2>
                        <div class="d-flex">
                            <i class="mdi mdi-file menu-icon"></i>
                            <p class="text-muted mb-0 hover-cursor">&nbsp;/&nbsp;Laporan&nbsp;/&nbsp;</p>
                            <p class="text-primary mb-0 hover-cursor">Laporan Penyelenggaraan Pemerintah</p>
                        </div>
                    </div>
                </div>
            </div>
                    ...
                    <div class="card-body">
                        ...
                        <div class="table-responsive">
                            <table class="table table-striped" id="data-tables2">
                                ...
                                <tbody>
                                    @foreach ($penyelenggaraans as $data)
                                        <tr>
                                            ...
                                            {{-- @if (auth()->check() && auth()->user()->name === 'admin') --}}
                                                <td>
                                                    <div class="d-flex justify-content-center">
                                                        <a href="{{ route('laporan_penyelenggaraan.edit', ['laporan_penyelenggaraan' => $data->id]) }}" class="btn btn-sm btn-warning text-white py-1 mr-1">
                                                            <span class="mdi mdi-pen"></span>
                                                        </a>
                                                        <form id="deleteForm-{{ $data->id }}" action="{{ route('laporan_penyelenggaraan.destroy', ['laporan_penyelenggaraan' => $data->id]) }}" method="POST" class="m-0">
                                                            @csrf
                                                            @method('DELETE')
                                                            <button type="button" class="btn btn-sm btn-danger text-white py-1" data-toggle="modal" data-target="#confirmModal" data-status="{{ $data->keterangan }}" data-id="{{ $data->id }}">
        ...
        <!-- Modal -->
            <div class="modal fade" id="addModal" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
                <form id="form" action="{{ route('laporan_penyelenggaraan.store') }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    <div class="modal-dialog modal-lg">
                        <div class="modal-content">
                            <div class="modal-header">
                                <h5 class="modal-title" id="exampleModalLabel">Tambah Data Laporan Kinerja Pemerintah</h5>
                            ...
                            <div class="modal-body">
                                <div class="form-group">
                                    ...
                                    <input type="text" class="form-control" id="keterangan" name="keterangan" placeholder="Masukkan keterangan laporan" required>
                                </div>
                                <div class="form-group">
                                    <label for="tahun">Tahun</label>
                                    <input type="number" class="form-control" id="tahun" name="tahun" placeholder="Masukkan tahun laporan" required>
                                </div>
                                <div class="form-group">
