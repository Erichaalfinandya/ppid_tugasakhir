...
@section('content')
            ...
            <div class="d-flex justify-content-between flex-wrap">
                <div class="d-flex align-items-end flex-wrap">
                    <div class="me-md-3 me-xl-5">
                        <h2>LAPORAN KINERJA PEMERINTAH- Edit data</h2>
                        <div class="d-flex">
                            <i class="mdi mdi-file menu-icon"></i>
                            <p class="text-muted mb-0 hover-cursor">&nbsp;/&nbsp;Laporan&nbsp;/&nbsp;</p>
                            <p class="text-muted mb-0 hover-cursor">&nbsp;/&nbsp;LAPORAN KINERJA PEMERINTAH&nbsp;/&nbsp;</p>
    ...
    <div class="card">
        <div class="card-body">
            <form id="form" action="{{ route('laporan_kinerja.update', ['laporan_kinerja' => $laporankinerja->id]) }}" method="POST" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                <div class="form-group">
                    <label for="keterangan">Keterangan</label>
                    <input type="text" class="form-control" id="keterangan" placeholder="Masukkan Keterangan/Judul Laporan"
                        name="keterangan" value="{{ old('keterangan', $laporankinerja->keterangan) }}" required>
                    @error('keterangan')
                ...
                <div class="form-group">
                    ...
                    <input type="text" class="form-control" id="tahun" placeholder="Masukkan Tahun Laporan"
                        name="tahun" value="{{ old('tahun', $laporankinerja->tahun) }}" required>
                    @error('tahun')
                        <div class="alert alert-danger">{{ $message }}</div>
                    @enderror
                </div>
                <div class="form-group">
                    ...
                    <input type="text" class="form-control" id="penerbit" placeholder="Masukkan Nama Penerbit"
                        name="penerbit" value="{{ old('penerbit', $laporankinerja->penerbit) }}" required>
                    @error('penerbit')
                        <div class="alert alert-danger">{{ $message }}</div>
                    @enderror
                </div>
