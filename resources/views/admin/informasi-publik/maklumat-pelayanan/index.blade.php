    ...
    <div class="row">
            ...
            <div class="d-flex justify-content-between flex-wrap">
                <div class="d-flex align-items-end flex-wrap">
                    <div class="me-md-3 me-xl-5">
                        <h2>Daftar Maklumat Pelayanan</h2>
                        <div class="d-flex">
                            <i class="mdi mdi-headset menu-icon"></i>
                            <p class="text-muted mb-0 hover-cursor">&nbsp;/&nbsp;Layanan&nbsp;/&nbsp;</p>
                            <p class="text-primary mb-0 hover-cursor">Daftar Maklumat Pelayanan</p>
                        </div>
                    </div>
                                ...
                                <tbody>
                                    @foreach ($list as $datas)
                                        <tr>
                                            ...
                                            <td class="text-center"><a href="{{ asset('storage/' . $data->file) }}" target="blank"
                                                    class="btn btn-success btn-sm text-white"><span class="mdi mdi-download"></span>
                                                    Download</a>
                                            </td> --}}
                                            @if (auth()->check() && auth()->user()->name === 'admin')
                                            <td>
                                                    <div class="d-flex justify-content-center">
                                                        <a href="{{ route('maklumat-pelayanan.edit', ['maklumat_pelayanan' => $datas->id]) }}"
                                                            type="submit" class="btn btn-sm btn-warning text-white py-1 mr-1">
                                                            <span class="mdi mdi-pen"></span>
                                                        </a>
                                                        <form id="deleteForm-{{ $datas->id }}" action="{{ route('maklumat-pelayanan.destroy', ['maklumat_pelayanan' => $datas->id]) }}" method="POST" class="m-0">
                                                            @csrf
                                                            @method('DELETE')
        ...
        @if (auth()->check() && auth()->user()->name === 'admin')
            ...
            <div class="modal fade" id="addModal" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
                <form action="{{ route('maklumat-pelayanan.store') }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    @method('POST')
            ...
            <div class="modal fade" id="editModal" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
                <form action="{{ route('maklumat-pelayanan.update_nolist') }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    @method('POST')
                    <div class="modal-dialog modal-lg">
                        <div class="modal-content">
                            ...
                            <div class="modal-body">
                                <div class="form-group">
                                    ...
                                    <input type="text" class="form-control"
                                        placeholder="Masukkan Judul" name="judul" required value="{{ $data->judul }}">
                                </div>
                                <div class="form-group">
                                    <label for="exampleFormControlInput1">Deskripsi</label>
                                    <textarea class="form-control" id=""
                                        placeholder="Masukkan Deskripsi" name="deskripsi" required>{{ $data->deskripsi }}</textarea>
                                </div>
                                <div class="form-group">
                                ...
                                <div class="form-group">
                                    <label for="exampleFormControlInput1">Link atau Tautan</label>
                                    <input type="text" class="form-control"
                                        placeholder="Masukkan Link" name="link" required value="{{ $data->link }}">
                                </div>
                            </div>
                            <div class="modal-footer">
                                <button type="button" class="btn btn-secondary" data-dismiss="modal">Tutup</button>
