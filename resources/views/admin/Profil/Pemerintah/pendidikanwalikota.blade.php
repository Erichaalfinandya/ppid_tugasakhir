    ...
    <div class="row">
            ...
            <div class="d-flex justify-content-between flex-wrap">
                <div class="d-flex align-items-end flex-wrap">
                    <div class="me-md-3 me-xl-5">
                        <h2> Riwayat Pendidikan Walikota Surakarta</h2>
                        <div class="d-flex">
                            <i class="mdi mdi-file menu-icon"></i>
                            <p class="text-muted mb-0 hover-cursor">&nbsp;/&nbsp;Riwayat Pendidikan&nbsp;/&nbsp;</p>
                            <p class="text-primary mb-0 hover-cursor">Walikota Surakarta</p>
                        </div>
                    </div>
                </div>
            </div>
        ...
        <div class="row mt-2 py-3 justify-content">
            <div class="col-auto">
                ...
                <a href="{{ Route('posisi_walikota.index') }}" class="btn btn-primary" style="margin-right: 10px;">
                    Posisi Walikota
                </a>
            </div>
            <div class="col-auto">
                <!-- Tombol untuk halaman pendidikan walikota -->
                <a href="{{ Route('pendidikan_walikota.index') }}" class="btn btn-secondary">
                    Pendidikan Walikota
                </a>
            </div>
        </div>
                            ...
                            <table class="table table-striped" id="data-tables2">
                                <thead class="text-center">
                                    <tr>
                                        ...
                                        <th scope="col" class="align-middle text-center">Pendidikan</th>
                                        ...
                                        <th scope="col" class="align-middle text-center">Sampul Pendidikan</th>
                                ...
                                <tbody>
                                    @foreach ($pendidikanwalikota as $pendidikan)
                                        <tr>
                                            <td class="text-center">{{ $loop->iteration }}</td>
                                            <td>{{ $pendidikan->pendidikan }}</td>
                                            <td>{{ $pendidikan->tahun }}</td>
                                            <td class="text-truncate" style="max-width: 250px;">{{ $pendidikan  ->deskripsi }}</td>
                                            <td class="text-center">
                                                <img src="{{ asset($pendidikan->sampul) }}" style="width: 75px; height: 75px; border-radius: 0%" alt="">
                                            </td>
                                            <td>{{ $pendidikan->penerbit }}</td>
                                            @if (auth()->check() && auth()->user()->name === 'admin')
                                                <td>
                                                    <div class="d-flex justify-content-center">
                                                        <a href="{{ route('pendidikan_walikota.edit', ['pendidikan_walikota' => $pendidikan->id]) }}" class="btn btn-sm btn-warning text-white py-1 mr-1">
                                                            <span class="mdi mdi-pen"></span>
                                                        </a>
                                                        <form id="deleteForm-{{ $pendidikan->id }}" action="{{ route('pendidikan_walikota.destroy', ['pendidikan_walikota' => $pendidikan->id]) }}" method="POST" class="m-0">
                                                            @csrf
                                                            @method('DELETE')
                                                            <button type="button" class="btn btn-sm btn-danger text-white py-1" data-toggle="modal" data-target="#confirmModal" data-status="{{ $pendidikan->pendidikan }}" data-id="{{ $pendidikan->id }}">
        ...
        <!-- Modal -->
            <div class="modal fade" id="addModal" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
                <form id="form" action="{{ route('pendidikan_walikota.store') }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    <div class="modal-dialog modal-lg">
                        <div class="modal-content">
                            <div class="modal-header">
                                <h5 class="modal-title" id="exampleModalLabel">Tambah Data Riwayat Pendidikan Walikota</h5>
                            ...
                            <div class="modal-body">
                                <div class="form-group">
                                    ...
                                    <input type="text" class="form-control" id="pendidikan" name="pendidikan" placeholder="Masukkan data riwayat pendidikan walikota surakarta" required>
                                </div>
                                <div class="form-group">
                                    <label for="tahun">Tahun Pendidikan</label>
                                    <input type="text" class="form-control" id="tahun" name="tahun" placeholder="Masukkan tahun pendidikan" required>
                                </div>
                                <div class="form-group">
                                    <label for="deskripsi">Deskripsi</label>
                                    <textarea class="form-control" id="deskripsi" name="deskripsi" placeholder="Masukkan deskripsi riwayat pendidikan"></textarea>
                                </div>
                                <div class="form-group">
                                    <label for="sampul">Sampul Pendidikan</label>
