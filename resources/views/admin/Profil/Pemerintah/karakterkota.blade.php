    ...
    <div class="row">
            ...
            <div class="d-flex justify-content-between flex-wrap">
                <div class="d-flex align-items-end flex-wrap">
                    <div class="me-md-3 me-xl-5">
                        <h2>Karakter Kota Pemerintah Surakarta</h2>
                        <div class="d-flex">
                            <i class="mdi mdi-file menu-icon"></i>
                            <p class="text-muted mb-0 hover-cursor">&nbsp;/&nbsp;Karakter Kota&nbsp;/&nbsp;</p>
                            <p class="text-primary mb-0 hover-cursor">Pemerintah Surakarta</p>
                        </div>
                    </div>
                </div>
            </div>
                            ...
                            <table class="table table-striped" id="data-tables2">
                                <thead class="text-center">
                                    <tr>
                                        ...
                                        <th scope="col" class="align-middle text-center">Deskripsi Kategori</th>
                                ...
                                <tbody>
                                    @foreach ($karakterkota as $karakter)
                                        <tr>
                                            ...
                                            <td>{{ $karakter->kategori }}</td>
                                            <td>{{ $karakter->deskripsi }}</td>
                                            <td>{{ $karakter->penerbit }}</td>
                                            {{-- @if (auth()->check() && auth()->user()->name === 'admin') --}}
                                                <td>
                                                    <div class="d-flex justify-content-center">
                                                        <a href="{{ route('karakter_kota.edit', ['karakter_kota' => $karakter->id]) }}" class="btn btn-sm btn-warning text-white py-1 mr-1">
                                                            <span class="mdi mdi-pen"></span>
                                                        </a>
                                                        <form id="deleteForm-{{ $karakter->id }}" action="{{ route('karakter_kota.destroy', ['karakter_kota' => $karakter->id]) }}" method="POST" class="m-0">
                                                            @csrf
                                                            @method('DELETE')
                                                            <button type="button" class="btn btn-sm btn-danger text-white py-1" data-toggle="modal" data-target="#confirmModal" data-status="{{ $karakter->deskripsi_karakter }}" data-id="{{ $karakter->id }}">
        ...
        <!-- Modal -->
            <div class="modal fade" id="addModal" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
                <form id="form" action="{{ route('karakter_kota.store') }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    <div class="modal-dialog modal-lg">
                        <div class="modal-content">
                            <div class="modal-header">
                                <h5 class="modal-title" id="exampleModalLabel">Tambah Data Karakter Kota Surakarta</h5>
                            ...
                            <div class="modal-body">
                                <div class="form-group">
                                    ...
                                    <input type="text" class="form-control" id="kategori" name="kategori" placeholder="Masukkan kategori karakter kota" required>
                                </div>
                                <div class="form-group">
                                    <label for="deskripsi">Deskripsi Kategori</label>
                                    <textarea class="form-control" id="deskripsi" name="deskripsi" placeholder="Masukkan deskripsi kategori karakter kota"></textarea>
                                </div>
                                <div class="form-group">
