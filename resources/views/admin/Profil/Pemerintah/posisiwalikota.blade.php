    ...
    <div class="row">
            ...
            <div class="d-flex justify-content-between flex-wrap">
                <div class="d-flex align-items-end flex-wrap">
                    <div class="me-md-3 me-xl-5">
                        <h2> Riwayat Posisi Walikota Surakarta</h2>
                        <div class="d-flex">
                            ...
                            <table class="table table-striped" id="data-tables2">
                                <thead class="text-center">
                                    <tr>
                                        ...
                                        <th scope="col" class="align-middle text-center">Posisi</th>
                                        <th scope="col" class="align-middle text-center">Riwayat Posisi/Lokasi Penempatan</th>
                                        <th scope="col" class="align-middle text-center" style="width: 250px;">Deskripsi</th>
                                        <th scope="col" class="align-middle text-center">Sampul Posisi</th>
                                ...
                                <tbody>
                                    @foreach ($posisiwalikota as $posisi)
                                        <tr>
                                            <td class="text-center">{{ $loop->iteration }}</td>
                                            <td>{{ $posisi->posisi }}</td>
                                            <td>{{ $posisi->lokasi }}</td>
                                            <td class="text-truncate" style="max-width: 250px;">{{ $posisi->deskripsi }}</td>
                                            <td class="text-center">
                                                <img src="{{ asset($posisi->sampul) }}" style="width: 75px; height: 75px; border-radius: 0%" alt="">
                                            </td>
                                            <td>{{ $posisi->penerbit }}</td>
                                            @if (auth()->check() && auth()->user()->name === 'admin')
                                                <td>
                                                    <div class="d-flex justify-content-center">
                                                        <a href="{{ route('posisi_walikota.edit', ['posisi_walikota' => $posisi->id]) }}" class="btn btn-sm btn-warning text-white py-1 mr-1">
                                                            <span class="mdi mdi-pen"></span>
                                                        </a>
                                                        <form id="deleteForm-{{ $posisi->id }}" action="{{ route('posisi_walikota.destroy', ['posisi_walikota' => $posisi->id]) }}" method="POST" class="m-0">
                                                            @csrf
                                                            @method('DELETE')
                                                            <button type="button" class="btn btn-sm btn-danger text-white py-1" data-toggle="modal" data-target="#confirmModal" data-status="{{ $posisi->posisi }}" data-id="{{ $posisi->id }}">
        ...
        <!-- Modal -->
            <div class="modal fade" id="addModal" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
                <form id="form" action="{{ route('posisi_walikota.store') }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    <div class="modal-dialog modal-lg">
                        <div class="modal-content">
                            <div class="modal-header">
                                <h5 class="modal-title" id="exampleModalLabel">Tambah Data Riwayat Posisi Walikota</h5>
                            ...
                            <div class="modal-body">
                                <div class="form-group">
                                    ...
                                    <input type="text" class="form-control" id="posisi" name="posisi" placeholder="Masukkan data riwayat posisi walikota kota surakarta" required>
                                </div>
                                <div class="form-group">
                                    <label for="lokasi">Riwayat Posisi/Lokasi Penempatan</label>
                                    <input type="text" class="form-control" id="lokasi" name="lokasi" placeholder="Masukkan penempatan posisi atau lokasi riwayat posisi" required>
                                </div>
                                <div class="form-group">
                                    <label for="deskripsi">Deskripsi</label>
                                    <textarea class="form-control" id="deskripsi" name="deskripsi" placeholder="Masukkan deskripsi riwayat posisi"></textarea>
                                </div>
                                <div class="form-group">
