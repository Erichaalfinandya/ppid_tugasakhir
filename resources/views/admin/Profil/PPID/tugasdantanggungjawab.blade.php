    ...
    <div class="row">
            ...
            <div class="d-flex justify-content-between flex-wrap">
                <div class="d-flex align-items-end flex-wrap">
                    <div class="me-md-3 me-xl-5">
                        <h2>Profil Tugas & Tanggung Jawab</h2>
                        <div class="d-flex">
                            <i class="mdi mdi-account menu-icon"></i>
                            <p class="text-muted mb-0 hover-cursor">&nbsp;/&nbsp;Profil&nbsp;/&nbsp;</p>
                            <p class="text-primary mb-0 hover-cursor">&nbsp;/&nbsp;Profil Tugas & Tanggung Jawab&nbsp;/&nbsp;</p>
                        </div>
                    </div>
                                ...
                                <tbody>
                                    @foreach ($list as $datas)
                                        <tr>
                                            ...
                                            <td class="text-center"><img src="/{{ $datas->gambar }}"></td>
                                            <td class="text-center">{{ $datas->judul }}</td>
                                            <td>{{ $datas->deskripsi }}</td>
                                            {{-- @if (auth()->check() && auth()->user()->name === 'admin') --}}
                                            <td>
                                                    <div class="d-flex justify-content-center">
                                                        <a href="{{ route('profiltugastanggungjawab.edit', ['id' => $datas->id]) }}"
                                                            type="submit" class="btn btn-sm btn-warning text-white py-1 mr-1">
                                                            <span class="mdi mdi-pen"></span>
                                                        </a>
                                                        <form id="deleteForm-{{ $datas->id }}" action="{{ route('profiltugastanggungjawab.destroy', ['id' => $datas->id]) }}" method="POST" class="m-0">
                                                            @csrf
                                                            @method('DELETE')
                                                            <button type="button" class="btn btn-sm btn-danger text-white py-1" data-toggle="modal" data-target="#confirmModal" data-status="{{ $datas->judul }}" data-id="{{ $datas->id }}">
        ...
        {{-- @if (auth()->check() && auth()->user()->name === 'admin') --}}
            ...
            <div class="modal fade" id="addModal" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
                <form action="{{ route('profiltugastanggungjawab.store') }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    @method('POST')
                        ...
                        <div class="modal-content">
                            ...
                            <div class="modal-body">
                                ...
                                <div class="form-group">
                                    ...
                                    <input type="file" class="form-control form-control-file" id="fileInput" name="file" accept="image/*" required>
                                    <small id="fileHelp" class="form-text text-muted">Hanya file gambar yang diperbolehkan (jpg, jpeg, png, gif).</small>
                                </div>
                            </div>
...
@endif
    ...
    <script>
        ...
        // note : script untuk jenis file harus gambar
